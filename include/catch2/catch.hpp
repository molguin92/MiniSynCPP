...
#else
        template<typename Func, typename U>
        using MapFunctionReturnType = typename std::remove_reference<typename std::remove_cv<typename std::result_of<
            Func(U)>::type>::type>::type;
#endif
    ...
    private:
        ...
        void addPattern()
        {
            ...
            for (std::size_t i = 0; i < m_escapeChars.size(); ++i)
                token =
                    token.substr(0, m_escapeChars[i] - m_start - i) + token.substr(m_escapeChars[i] - m_start - i + 1);
            m_escapeChars.clear();
...
namespace Catch
{
    namespace Matchers
    {
        ...
        namespace StdString
        {
            ...
            RegexMatcher::RegexMatcher(std::string regex, CaseSensitive::Choice caseSensitivity) : m_regex(std::move(
                regex)), m_caseSensitivity(caseSensitivity)
            {}
...
// Implementation of CompactReporter formatting
    ...
    void CompactReporter::sectionEnded(SectionStats const& _sectionStats)
    {
        ...
        {
            stream
                << getFormattedDuration(_sectionStats.durationInSeconds)
                << " s: "
